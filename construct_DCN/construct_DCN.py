...
def get_onehot_disea(df_p_d):
    """
    构建patient - disease矩阵
    :param df_p_d: 包含ALL_DISEASE和SFZH字段的DataFrame
    :return: df_p_d:patient - disease矩阵; 一行表示一个患者，列方向上包括number_of_diseases个字段，记录是否患有某个疾病
    :return: dict_d: 字典，{疾病ICD10编码：列编号}
    """
    def get_onehot_d(df_group,dict_d):
        ...
        for i in df_group['ALL_DISEASE']:
            for j in i:
                tmp[dict_d[j]] = 1
        return df_group['SFZH'].values[0], tmp

    dict_d=set()
    for i in df_p_d['ALL_DISEASE']:
        dict_d=dict_d|i
    dict_d = np.sort(np.array(list(dict_d)))
    dict_d=dict(zip(dict_d,range(dict_d.shape[0])))  # 所有疾病-编号的字典

    df_p_d=df_p_d.groupby(['SFZH']).apply(get_onehot_d, dict_d)
    df_p_d=dict(df_p_d.values)
    df_p_d = pd.DataFrame(zip(df_p_d.keys(), df_p_d.values()), columns=['SFZH', 'lst_disea_binary'])
...
def construct_sparse_matrix(p_d_nda, dict_all_d,is_chronic):
    """
    输入参数p_d_nda[['SFZH','diseases']]; dict_all_d: 疾病-编号的字典; is_chronic: 是否去除急性病
    输出：p_d_nda, dict_all_d
    功能：根据从数据库读入的数据生成患者-慢病稀疏矩阵
    """
    # 是否去除急性病  （ is_chronic==1表示去除急性病）
    if is_chronic==1:
...
def choose_diseases_based_on_matrix(p_d_nda, dict_selected_d, has_IHD, prevalence_threshold, lst_research_d):
    ...
    输入参数：
        ...
        _has_IHD： has_IHD==0，则去除索引疾病(如I20-I25），即共病网络不考虑这六种疾病 ; has_IHD==1,则将I20-I25纳入共病网络
        _prevalence_threshold:选取的疾病的流行率下限
        lst_research_d: 索引疾病（比如研究冠心病群体的共病情况，索引疾病即为I20-I25）
...
def compute_Cij(dic_cols,csc_matrix_final):
    """
    计算Cij矩阵,
    Cij矩阵是一个上三角矩阵，第i行第j列的元素表示同时患i和j两种疾病的人数，主对称轴元素均为0
...
if __name__ == '__main__':
    ...
    if 2 in process:
        # 构建patient - disease矩阵（p_d_nda）
        # p_d_nda： 行表示一个患者，列表示onehot格式的疾病
        ...
        print('1. p_d_nda.shape,len(dict_all_d)', p_d_nda.shape, len(dict_all_d))

    if 3 in process:
        # 根据纳入排除标准，去除快病，构建患者-疾病稀疏矩阵
    ...
    if 4 in process:
        # 根据纳入排除标准，去除患病率小于prevalence_threshold的疾病，更新患者-慢病稀疏矩阵
    ...
    if 5 in process:
        ...
        # 计算疾病之间的相关性，此处提供了如下三种相关性度量指标：'phi', 'RR', 'CC'
