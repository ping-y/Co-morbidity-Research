...
import _3_forIndexDis
import _2_forMultimobidity
import random
...
if __name__ == "__main__":
    ...
    df_patient_case = pickle.load(open("df_patient_for_IndexDis_%s.pkl" % (index_dis), 'rb'))

    # 匹配候选队列
    df = pickle.load(open("df.pkl", 'rb'))

    ls_index_dis = _3_forIndexDis.get_index_dis(index_dis)
    set_index_dis = set(ls_index_dis)
    df['has_index_dis'] = df['ALL_DISEASE'].apply(lambda x: len(x & set_index_dis))
    ctrl_sfzhs = pd.DataFrame(list(set(list(df['SFZH']))-set(list(df[df['has_index_dis'] > 0]['SFZH']))), columns=['SFZH'])
    df_ctrl=pd.merge(ctrl_sfzhs, df, on=['SFZH'])

    df_patient_ctrl = _2_forMultimobidity.collect_disease(df_ctrl)  # 合并多次住院  ['SFZH', 'NL', 'XB', 'XZZ_XZQH2', 'CY_DATE', 'ALL_DISEASE']
    # 存储 df_patient_ctrl: 一人一行，包括字段：['SFZH', 'NL', 'XB', 'XZZ_XZQH2','CY_DATE', 'ALL_DISEASE']
    pickle.dump(df_patient_ctrl, open("df_patient_ctrl_%s.pkl"%(index_dis), 'wb'), protocol=4)

    # ===========================================  根据df_patient_case和df_patient_ctrl完成匹配
    ...
    df_patient_case['is_city']=df_patient_case['XZZ_XZQH2'].apply(lambda x: dic_code2city[x[:6]] if x[:6] in dic_code2city else 'nan')
    df_patient_ctrl['is_city'] = df_patient_ctrl['XZZ_XZQH2'].apply(lambda x: dic_code2city[x[:6]] if x[:6] in dic_code2city else 'nan')
    ...
    df_controlp = pd.DataFrame([list(i) for i in df_controlp], columns=['NL','XB', 'CY_YEAR', 'CY_MONTH', 'is_city', 'SFZHs'])
    ...
    for block_i in tqdm(range(block_num)):   # 切分case df，分治；不然merge后得到的df_match所需存储空间会溢出内存
        ...
        df_casep_portion = df_casep.iloc[int(block_i / block_num * df_casep.shape[0]): int((block_i + 1) / block_num * df_casep.shape[0]), :]

        caseid_ctrlid_0 = pd.merge(df_casep_portion, df_controlp, on=['NL', 'XB', 'CY_YEAR', 'CY_MONTH', 'is_city'], how='left')
        ...
        for nl_range in nl_ranges:
            ...
            caseid_ctrlid = pd.merge(df_casep_portion, df_controlp, on=['NL', 'XB', 'CY_YEAR', 'CY_MONTH', 'is_city'], how='left')
            df_casep_portion['NL'] = df_casep_portion['NL'] - nl_range
    ...
    pickle.dump(df_ctrl_sfzhs, open("df_ctrl_sfzhs_%s.pkl"%(index_dis), 'wb'), protocol=4)

    # 可进一步对存储的df_patient_ctrl进行选取
    df_patient_ctrl_final=pd.merge(df_ctrl_sfzhs, df_patient_ctrl, on=['SFZH']).reset_index(drop=True)
    pickle.dump(df_patient_ctrl_final, open("df_patient_ctrl_final_%s.pkl" % (index_dis), 'wb'), protocol=4)
